    ...
    <div>
      ...
      <div class="ripple py-2 px-5" align="left"
        ...
        コンテナNo
          ...
          <div  style="margin: 0 auto 2rem;text-align: left;font-size: 60px">          
            <va-button v-for="findcontainers in findcontainers" :key="findcontainers" v-model="container.containerName" @click="selectbutton(findcontainers)" style="width:100%;height:70px" > 
              <div style="text-align: center;font-size:45px">
              {{ findcontainers.uldNo }}
              </div>
              </va-button>   
          </div>
           <div v-if="resultarea">
...
mounted: function (){ 
 /* this.termNoFileCheck();
  if (this.termNoFileExists === true) {
      ...
      this.axios({
        url: '/api/oaew003/searchContainer',
        method: 'get',
      })    
         .then(res => {
          if(res.data.result){
          this.findcontainers = res.data.resultData
          } else {
        ...
        this.container.flightName = this.$route.params.flightName
    },
methods: {
   selectbutton: function (findcontainers) {
      this.select = findcontainers;
      this.flightName = this.container.flightName 
 this.$router.push({ name: 'OAEW003stowage', params: {flightName:this.flightName, containerName: findcontainers.uldNo } })
  },
  termNoFileCheck(){
      /* eslint-disable */
      ...
      try {
        ...
        this.container.termNo = KJS.File.readLine(fp);
        console.log(this.container.termNo);
        KJS.File.close(fp);
    ...
    goBack() {
      this.$router.push({ name: 'OAEW003flightselect'})
    }
 }
}
</script>