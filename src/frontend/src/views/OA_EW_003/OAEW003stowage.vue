  ...
  <div class="grid md:grid-cols-1 sm:grid-cols-1 gap-2">
    <div>
      ...
      <div style="margin-bottom: 5px;text-align: right;font-size: 25px;margin-right: 10px">
        ...
        <va-input style = "width:10px" v-if="!termNoFileExists" v-model="schCondition.termNo" :disabled="!isEditable" v-on:keydown.enter="update()" />
      ...
      <div class="ripple py-2 px-5" align="left"
           style="width:100%;height:30px;font-size: 25px">
     {{ schCondition.containerName}}
    </div> 
      <div class="ripple py-2 px-5" align="left"
      ...
      <va-input ref="numberBox" style="width: 100% ; margin-bottom: 10px;font-size: 30px" v-model="schCondition.cwbNo" v-on:keydown.enter="cargoCheck(schCondition)" />          
    </div>
    </va-inner-loading>
    </va-card>
    <div v-if="resultarea">
...
export default {
  ...
  this.termNoFileCheck();
    ...
    this.searchWarehouse();    // DBからUSERCDの取得を行う
    }
  console.log(this.$route.params)
  this.schCondition.containerName = this.$route.params.containerName
  this.schCondition.fltDestination = this.$route.params.flightName
  ...
  methods: {
    // 貨物情報の確認、DBの貨物情報更新、共通部品へステータスを伝送
    cargoCheck(schCondition) {
      ...
      this.axios({
        ...
        this.schCondition.cwbNo = '';
        this.changeLoading();
        if(this.resultDisplay === "積付完了"){
         this.cardClass = "#2d8524";
         const audioError = new Audio(require("./Audio/Ok.mp3"));
          audioError.play();
        }else if(this.resultDisplay === "積付済み"){
         this.cardClass = "#DDEE44";
         this.$nextTick(() => {
         this.$refs.numberBox.focus();})
        }else if(this.resultDisplay ==="コンテナ間違い"||this.resultDisplay ==="未許可の貨物"){
     ...
     termNoFileCheck(){
      ...
      this.axios({
       ...
       goBack: function () {
        const flightName = this.schCondition.fltDestination
        this.$router.push({ name: 'OAEW003containerregist', params:{flightName}})
  },
   changeLoading() {
