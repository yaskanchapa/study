...
<template>
  <div class="grid md:grid-cols-1 sm:grid-cols-1 gap-4">
    <div>
      ...
      <div class="ripple py-2 px-4" align="left"
      ...
      <div class="ripple py-2 px-5" align="left"
        ...
        積込港選択
          ...
          <div  style="margin: 0 auto 2rem;text-align: left;font-size: 60px;">     
            <va-button v-for="findports in findports" :key="findports"  v-model="port.portName" @click="selectbutton(findports)" type="submit" style="width:100%;height:70px;font-size: 20px" >
...
mounted: function () {
 /* this.termNoFileCheck();
  if (this.termNoFileExists === true) {
      ...
      this.axios({
        url: '/api/oaew003/searchPort',
        method: 'get',
      })    
         .then(res => {
          if(res.data.result){
          this.findports = res.data.resultData
          this.portName = res.data.portName
          } else {
 ...
 methods: {
   selectbutton: function (findports) {
      this.select = findports;
 this.$router.push({ name: 'OAEW003examination', params: { portName: findports.nameCd } })
  },
  termNoFileCheck(){
      /* eslint-disable */
      ...
      try {
        ...
        this.port.termNo = KJS.File.readLine(fp);
        console.log(this.port.termNo);
        KJS.File.close(fp);
