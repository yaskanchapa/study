  ...
  <div class="grid md:grid-cols-1 sm:grid-cols-1 gap-4">
    ...
    <va-card style="margin-left: 10px; margin-right: 10px">
      ...
      <div style="text-align: left;margin-left: 10px; margin-right: 10px">
        ...
        <va-button style="width: 10% ; margin-left: 10px;font-size: 30px" @click = "invMawbCheck(schCondition)">更新</va-button>
      </div>
      <div style="margin-left: 10px; margin-right: 10px; font-size: 25px">
        <tbody>
        <tr>
      ...
      <div v-if="multipleDataPicec">
        ...
        <div style="text-align: left;margin-left: 10px; margin-right: 10px">
          <va-input ref="sPicec" style="width: 80%; margin-bottom: 10px;font-size: 30px"  v-model="schCondition.inputScanPicec" mask="numeral" placeholder = "複数個口の貨物数入力" v-on:keydown.enter="bondedInvCheck(schCondition)"/>
...
export default {
  ...
  mounted() {
    ...
    if (this.termNoFileExists === true) {
      ...
      this.invMawbCheck(this.schCondition);
    }
    // this.searchWarehouse();
    // this.invMawbCheck(this.schCondition);
  },
  methods: {
    ...
    checkDataPicec(schCondition) {
      ...
      if (schCondition.termNo == null || schCondition.termNo === '') {
        ...
        this.bondedInvCheck(schCondition);
        ...
        this.axios({
          url: '/api/oaiw004/checkDataPicecInv',
          method: 'get',
          params: schCondition,
          enctype: 'multipart/form-data'
        })
          .then(res => {
            ...
            if (res.data == null){
              this.bondedInvCheck(schCondition);
            } else if (res.data.awbno !== this.schCondition.mawbNo){
              console.log("diffMaster");
              this.bondedInvCheck(schCondition);
            } else if (res.data.datapicec > 1 && res.data.overflg === "0") {
    ...
    bondedInvCheck(schCondition) {
      console.log('schCondition', schCondition);
      if (schCondition.userNo === '') {
        console.log("No User ID");
      }
      this.axios({
        url: '/api/oaiw004/searchInv',
        method: 'get',
        params: schCondition,
        enctype: 'multipart/form-data'
      })
        .then(res => {
          ...
          this.resultAudioPath = res.data.audiopath.toString();
          console.log(res.data.scanresult);
        ...
        } else if (this.resultDisplay === "データ無し" ||
