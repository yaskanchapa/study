  ...
  <div class="grid md:grid-cols-1 sm:grid-cols-1 gap-4">
    ...
    <va-card style="margin-left: 10px; margin-right: 10px">
      ...
      <div style="text-align: left;margin-left: 10px; margin-right: 10px">
        ...
        <va-button style="width: 10% ; margin-left: 10px;font-size: 30px" @click = "bondedOutMawbCheck(schCondition)">更新</va-button>
      </div>
      <div style="margin-left: 10px; margin-right: 10px; font-size: 25px">
        <tbody>
        <tr>
      ...
      <div style="text-align: center;margin-left: 10px; margin-right: 10px">
        <va-input style="width: 100%; margin-bottom: 10px;font-size: 30px" v-model="schCondition.hawbNo" placeholder="ハウス番号" v-on:keydown.enter="checkDataPicec(schCondition)" />
      </div>
    ...
    <div v-if="multipleDataPicec">
      ...
      <div style="text-align: left;margin-left: 10px; margin-right: 10px">
        <va-input ref="sPicec" style="width: 80%; margin-bottom: 10px;font-size: 30px"  v-model="schCondition.inputScanPicec" mask="numeral" placeholder = "複数個口の貨物数入力" v-on:keydown.enter="bondedOutCheck(schCondition)"/>
...
export default {
  ...
  mounted() {
    ...
    if (this.termNoFileExists === true) {
      ...
      this.bondedOutMawbCheck(this.schCondition);
    }
  },
  methods: {
    checkDataPicec(schCondition) {
      ...
      if (schCondition.termNo == null || schCondition.termNo === '') {
        ...
        this.bondedOutCheck(schCondition);
        ...
        this.axios({
          url: '/api/oaiw004/checkDataPicecOut',
          method: 'get',
          params: schCondition,
          ...
          .then(res => {
            ...
            console.log(Number(res.data.overflg))
            if (res.data.awbno.toString() !== this.schCondition.mawbNo.toString()) {
              console.log("diffMaster");
              this.bondedOutCheck(schCondition);
            } else if (Number(res.data.datapicec) > 1 && Number(res.data.overflg) === 0) {
    ...
    bondedOutCheck(schCondition) {
      console.log('schCondition', schCondition);
      this.axios({
        url: '/api/oaiw004/searchOut',
        method: 'get',
        params: schCondition,
        enctype: 'multipart/form-data'
      })
        ...
        } else if (this.resultDisplay === "オーバー" ||
          this.resultDisplay === "フライト違い" ||
          this.resultDisplay === "未申告" ||
          this.resultDisplay === "未許可" ||
          this.resultDisplay === "未搬入" ||
          this.resultDisplay === "検査"
        ) {
          this.resultGreen = false;
          this.resultRed = true;
          this.resultBlack = false;
    ...
    bondedOutMawbCheck(schCondition){
      this.resultGreen = false;
      this.resultRed = false;
      this.resultBlack = false;
      console.log('schCondition', schCondition);
